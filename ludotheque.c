...
Description générale :
	...
	-Si le type de jeu n'est pas trouvé ou si les stocks sont vides, on retourne le compteur.
...
int affichageJeuxDunType(Jeu *tJeux[], int nbJeux, char *typeJeu)
{
	...
	for (i = 0; i < nbJeux; i++)
	{
		...
		{
			printf("%-10s\t\t%-30s\t\t%d\n", tJeux[i]->typeJeu, 
				tJeux[i]->nomJeu, tJeux[i]->nbExemplaire);
			compteur++;
		}
	}
	return compteur;
...
Finalité : Fonction principale de l'affichage des réservations. C'est ici que l'utilisteur entre jeu et qu'on appelle la fonction d'affichage.
...
/*
Nom:ajoutDunEmprunt
  Description: Fonction de l'ajout d'un emprunt, la fonction recherche le nombre d'emprunt déjà effectuer par l'adherent si ce nombre est supérieur a 3
  l'emprunt ne s'effectue pas. La fonction recherche aussi si l'adhérent emprunte déjà un jeu depuis plus d'un mois si c'est le cas l'emprunt est aussi refusé,
  si toutes les conditions sont validé alors on copie toutes les informations importante dans le tableau emprunt on alloue dynamiquement le tableau, on recherche la date
  actuel enfin nous décalons a droite pour rajouter la valeur de le tableau de la structure emprunt enfin nous augmentons la taille logique du nombre d'emprunt de 1
  et nous renvoyons la valeur de l'id d'emprunt
  Paramètre en entrée:
  	-**tEmp  tableau de pointeur de la structure Adherent
  	-nbEmp taille logique du fichier emprunt
  	-idAdherent id de l'adhérent voulant faire l'emprunt d'un jeu
  	-idJeux  id du jeux que l'adhérent veut emprunter
  	-dateActu date du jour ou l'adhérent souhaite emprunté le jeu 
	
  Paramètre en sortie:
   -int entier permettant de constater si le programme c'est bien déroulé ainsi que l'id d'emprunt si celui ci c'est bien terminé
  Constantes:Pas de constante dans le programme.
  Variable locale:
  	-compteurEmp: variable qui renvoie le nombre d'emprunt dans le tableau Emprunt 
  	-Emprunt emp: 
*/
...
/*
Nom:decalageADroite
  Description:Fonction qui permet de décaler a droite le tableau Emprunt permet ensuite d'insertion des valeurs grâce a l'indice i (idEmprunt) dans une 
  fonction d'ajout
  Paramètre en entrée:
  -Emprunt **tEmp tableau de pointeur de la structure Emprunt
  -int nbEmp taille logique du fichier emprunt
  -int i indice récupéré de la recherche
	
  Paramètre en sortie:
   Aucune paramètre en sortie
  Constantes:
  	Aucune constante dans le programme
  Variable locale:
  	j, variable compteur servant pour le for 
*/
...
/*
Nom:ajoutDunAdherent
  Description: Fonction d'ajout d'un adhérent récupère les informations rentrée par le nouvel adhérent civilité nom prénom, vérification si la longueur 
  du prénom ou nom est satisfaisant.Si c'est le cas nous récupérons toutes les données pour les rajouter dans la structure (le tableau) enfin nous allons allouer
  dynamiquement la structure, nous augmenter la taille logique du nombre d'adhérent de 1 et nous renvoyons 0 mpour signifier que l'ajout c'est bien déroulé
  Paramètre en entrée:
  	-**tAdh tableau de pointeur de la structure Adherent
  	-*nbAdh pointeur de la taille logique du nombre d'adhérent dans le fichier adhérent
  Paramètre en sortie:
  -int entier permet de savoir si toutes les fonctions appelé ce sont bien déroulé et qu'il n'y est pas de message d'erreur.
  Constantes:
  -int longueurMax correspond a la longueur maximum de caractère que le nom ou prénom peut avoir
  Variables:
  int longueur correspond a la valeur récupéré par strlen pour connaître le nombre de caractère que possède l'adhérent
  dateActu: est composé de la date du jour grâce a la fonction qu'il appel
  Adherent adh: structure Adhérent
*/
...
/*
Nom:comparaison Dates
  Description:Fonction permettant de comparer 2 dates pour savoir si la condition est respecté renvoie 0 si la condition est bonne,1 si la condition
  n'est pas respecté et enfin 2, si aucune comparaison a été effectué. Permet de savoir qu'elle reservation a la date la plus ancienne selon toutes les conditions.
  Paramètre en entrée:
  -date1 correspond a la date de la réservation
  -date2 correspond à la date du jour
	
  Paramètre en sortie:
  -int entier de vérification si une date est supérieur a 1 ans
  Constantes:Pas de constante dans le programme.
*/
...
Description générale : Fonction de retour d'un jeu demande au client son numéro d'emprunt reçu pendant l'emprunt, on récupère ensuite l'id du jeu
  et l'idAdh en fonction du numéro d'emprunt, on recherche ensuite le nom,prénom de la personne ainsi que le nom du jeu,on demande ensuite
  une vérification du client pour ensuite lancer la suppresion de l'emprunt.
Paramètres en entrée:
	-**tJeux tableau de pointeur de la structure jeux
	-nbJeux taille logique du fichier nbJeux
	-**tAdh tableau de pointeur de la structure adhérent, permet de stocker les informations 
  	qu'il y a dans le fichier.
	-nbAdh taille logique du fichier Adhérent
	-nbEmp taille logique du fichier Emprunt
	-**tEmp tableau de pointeur de la structure Emprunt
Valeur retournée :
  	-renvoie la liste Reservation
Variables locales :
  -char supp caractère récupéré par le client pour savoir si il souhaite bien rendre le jeu
  -Jeu jouet changement de nom de jeu le nouveau nom sera envoyé en paramètre et récupéré en pointeur
  -Adherent adherent changement de nom de Adherent le nouveau nom sera envoyé en paramètre et récupéré en pointeur
  -Emprunt emp changement de nom de Emprunt le nouveau nom sera envoyé en paramètre et récupéré en pointeur
  -int idEmprunt constante de récupération de l'id Emprunt rentré précédemment par le client
  -int erreurJeu valeur de retour de la vérification si l'id du Jeu existe bien
  -int erreurInfo valeur de retour de la vérification si l'id du client existe bien
  -int present valeur de retour de la présence ou non de l'id Emprunt dans le fichier
*/
...
/*
Nom : transformationResaEnEmprunt
  Description:Fonction qui transforme un id de réservation en id d'emprunt lorsque une réservation est possible
	La fonction recherche la date la plus ancienne du fichier réservation, la fonction ajoute l'emprunt vérifie si l'ajout c'est bien passée réduit le stock
	du jeu de 1. supprime la réservation qui devient un emprunt.
Paramètres en entrée:
  -lr liste chaîné résarvation
  -**tAdh tableau de pointeur sur la structure adhérent
  -**tEmp tableau de pointeur sur la structure Emprunt
  -nbadh taille logique du nombre d'adhérent
  -*nbEmp pointeur sur la taille logique du nombre d'emprunt
  -**tJeux tableau de pointeur sur la structure jeu
  -positionJeu: entier de la position du jeu qui servira pour la transformation
Valeur retournée:
  -renvoie la liste Reservation
Variables locales:
  dateResa
  -erreur entier des erreur récupère par les différentes fonctions
  -idEmprunt entier correspondant au nouvel idEmprunt que le client aura

Constantes :
	-nbAnneeEleve, nbJourEleve, nbMoisEleve : valeurs d'initialisation très élevées des jours, mois et années
  */
...
{
	int nbJourEleve = 32, nbMoisEleve = 13, nbAnneeEleve = 30000;
	Date dateResa = {nbJourEleve, nbMoisEleve, nbAnneeEleve};
	Date dateActu = recupererDateActuelle();
	Reservation reserv;
	Adherent adh;
...
/*
Nom:suppressionEmprunt
  Description:Fonction de suppresion de l'emprunt rentré par le client, la fonction décale a gauche le tableau ce qui supprime l'id de l'emprunt ainsi que les informations de cette ligne à l'aide de free qui libère l'allocation dynamique de l'indice 
  Paramètre en entrée:
	-*emp pointeur sur la structure emprunt
	-indice valeur de recherche récupéré de la fonction précédente nous permettra de supprimer cette élément qui est l'élément rentré par le client
	-nbEmp taille logique du fichier Emprunt
	-**tEmp tableau de pointeur de la structure Emprunt
  Paramètre en sortie:
  	-Aucun paramètre en sortie	
  Variable locale:
  -int i, variable qui servira de compteur permet prendre la valeur de l'indice permet la suppresion de l'élément de i dans tEmp. 
*/
...
/*
Nom:suppressionReservation
  Description:
  Paramètre en entrée:
  	-**tJeux tableau de pointeur avec les informations du fichier jeu en fonction des donnée de la structure
  	-nbJeux taille logique du nombre de jeu du fichier jeu
  	-**tAdh tableau de pointeur avec les informations du fichier Adherent en fonction des donnée de la structure
  	-nbAdh taille logique du nombre d'adhérent du fichier adhérent
  	-ListeReservation lr une liste de réservation lr où des données vont être insérées.
  Paramètre en sortie:
  	-ListeReservation Une liste de réservation ou a été ajouté un maillon.	
  Constantes:
  	-int longueurMax entier de constante 30 permet de vérifier si la longueur d'un élément est pas supérieur a 30 caractère.
  	-Reservation reserv
  	-Adherent adh
  	-Jeu jouet
  Variables locale:
  	-int idResa variable qui récupère l'id de la Reservation rentré par l'utilisateur
  	-int erreur variable permettant de savoir si une des fonctions c'est bien déroulé
  	-int longueur variable récupéré par strlen, cette valeur est ensuite récupéré et comparé avec longueurMax
  	-char nomEntre variable qui récupère le nom de l'utilisateur pour vérifier si la bonne personne veut bien supprimer son annullement de réservation de 30
  	caractère maiximum.
  	-char choix variable qui récupère le choix de l'utilisateur permet de savoir si la suppression est confirmé.
...
Nom:suppressionEnTete
  Description:Fonction qui supprime la liste par le début renvoie a chaque fois le maillon suivant, suppression grâce a free.
  Paramètre en entrée:
  	-ListeReservation lr une liste de réservation lr ou des données cont être insérées.
  Paramètre en sortie:
  	-ListeReservation: liste de Réservation renvoyé la valeur suivante
  Constantes:
  -un maillon *svt, alloué, puis rempli de données, et qui pointe sur la liste.
*/
...
/*
Nom:suppressionDuneReservation
  Description:Fonction récurssive de suppresion d'une reservation renvoie la la liste si la liste est vide sinon compare l'id Reservation
  envoyé en entrée par l'id présent dans la structure liste, si l'id correspond on la supprime grâce a la fonction suppression en tête, le fichier
  est trié par rapport a l'id donc si l'id de la dépasse celui envoyé en entrée cela veut dire qu'il n'était pas présent et on peut donc renvoyé la liste
  Enfin la fonction étant récursive on la rapelle en avançant dans la liste.  
  Paramètre en entrée:
  	-ListeReservation lr une liste de réservation lr ou des données cont être insérées.
  	-int idResa correspond a l'id Reservation que l'on veut supprimer. 
  Paramètre en sortie:
  	-ListeReservation: liste de Réservation renvoyé la valeur suivante
  Constantes:
  Aucune constante.
